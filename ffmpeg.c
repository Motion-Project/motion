...
static int ffmpeg_set_pts(struct ffmpeg *ffmpeg, const struct timeval *tv1){
    ...
    if (ffmpeg->tlapse != TIMELAPSE_NONE) {
        ...
        if (ffmpeg->test_mode == 1){
            MOTION_LOG(INF, TYPE_ENCODER, NO_ERRNO, "ms interval %d PTS %d timebase %d-%d",pts_interval,ffmpeg->pkt.pts,ffmpeg->video_st->time_base.num,ffmpeg->video_st->time_base.den);
        }
        ...
        if (ffmpeg->pkt.pts <= ffmpeg->last_pts){
            ...
            if (ffmpeg->test_mode == 1){
                MOTION_LOG(INF, TYPE_ENCODER, NO_ERRNO, "BAD TIMING!! PTS reset to incremental counter new PTS %d ",ffmpeg->pkt.pts);
            }
        }
...
static int ffmpeg_put_frame(struct ffmpeg *ffmpeg, const struct timeval *tv1){
    ...
    if (retcd < 0) {
        MOTION_LOG(ERR, TYPE_ENCODER, NO_ERRNO, "Error while setting PTS");
        return -1;
    }
