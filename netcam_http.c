...
int header_extract_authinfo(const char *http_header, void *closure)
{
	...
	if(http_header!=0)
	{
		int size=strlen(http_header);
		if(size==0) return 0;
		buffer=mymalloc(size+16);
		if(buffer==0) return 0;
		strncpy(buffer,http_header,size);
	}
  ...
  char *s,*e,*header,*value;
	...
	while(p!=0 && p[0] > 0)
	{
		...
		p=e+1; //skip over "
		s=strchr(p,','); // set e to location of ,
		if(s!=0)
		{
			p=s+1; //skip over ,
			while(p[0] != 0 && (p[0]==' ' || p[0]=='\n'  )) p++; // skip spaces
		...
		if(strcmp(header,"Digest realm")==0)
		{
			strcpy(auth->server_realm,value);
		}
		else if(strcmp(header,"qop")==0)
		{
			strcpy(auth->server_qop,value);
		}
		else if(strcmp(header,"nonce")==0)
		{
			strcpy(auth->server_nonce,value);
		}
	}
	free(buffer);
	return 1;
...
int do_digest_auth(netcam_context_ptr netcam)
{
	...
	{     /* 'Do forever' */
		...
		MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("Received header ('%s')"), header);
		
		if (ret != HG_OK) {
			MOTION_LOG(WRN, TYPE_NETCAM, NO_ERRNO
								 ,_("Error reading header (%s)"), header);
			free(header);
			return 0;
		}
		digest_authinfo *info=&(netcam->server_digest_auth_info);
		
		if(header_process(header, "WWW-Authenticate", header_extract_authinfo, info))
		{
			MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("found WWW-Authenticate header"));
			MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("realm '%s'"),netcam->server_digest_auth_info.server_realm);
			free(header);
					//empty buffer
			while(rbuf_flush(netcam,0,255) > 0);
					//loop around and retry request with digest info
			return 1;
		}
		free(header);
	}
	...
	while (1) {     /* 'Do forever' */
		...
		if (firstflag) {
			if ((ret = http_result_code(header)) != 200) {
				...
				if(ret==401) //try digest auth
				{
					free(header);
					if(do_digest_auth(netcam)) //send digest auth info
						return -1;
				}
...
static int netcam_http_request(netcam_context_ptr netcam)
{
	...
	for (ix = 0; ix < MAX_HEADER_RETRIES; ix++) {
		...
		netcam_url_parse(&url, netcam->cnt->conf.netcam_url);
		netcam_http_build_url(netcam, &url);
		netcam_url_free(&url);
	}
...
static int netcam_http_build_url(netcam_context_ptr netcam, struct url_t *url)
{
	...
	char *authorization_header = NULL;        /* Temp storage digest authorizationfor info */
	int ix;
	...
	MOTION_LOG(INF, TYPE_NETCAM, NO_ERRNO
			...
			/*
			 * The network camera may require a ussername and password.  If
	...
	if (ptr) {
		...
		userpass = NULL;
			...
			/* if digest data is available make a digest header */
	if (
		strlen(netcam->server_digest_auth_info.server_realm)>0
		&& 
		strlen(netcam->server_digest_auth_info.server_nonce)>0
			)
	{
		if (cnt->conf.netcam_userpass)
			ptr = cnt->conf.netcam_userpass;
		...
		if (ptr) /* there's a username:password to use */
		{
			...
			char *p;
			userpass = mymalloc(strlen(ptr)+2); /* room for terminating 0 and a bit */
			strcpy(userpass,ptr);
			MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("userpass ('%s')"), userpass);
			p=strchr(userpass,':');
			if(p)
			{
				p[0]=0; // terminate username
				MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("userpass ('%s')"), userpass);
				password=p+1;
				MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("password ('%s')"), password);
				username=userpass;
				MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("username ('%s')"), username);

				headersize+=133; //unfilled header size
				headersize+=strlen(username);
				headersize+=strlen(netcam->server_digest_auth_info.server_realm);
				headersize+=strlen(netcam->server_digest_auth_info.server_nonce);
				headersize+=strlen(url->path);
				headersize+=8; //client nonce
				headersize+=16; //padding


				char client_nonce[9];
				srand(time(NULL));
				sprintf(client_nonce,"%08x",(int)rand()*0xFFFFFFFF);

				char nonce_count[9];
				sprintf(nonce_count,"%08x",++netcam->server_digest_auth_info.client_counter);
					
				HASH HA1;
				HASH HA2="";
				HASHHEX response;

				DigestCalcHA1((char*)"md5", username, netcam->server_digest_auth_info.server_realm,
											password, netcam->server_digest_auth_info.server_nonce,
											client_nonce, HA1);
	...
	if (userpass && !authorization_header) { /* only do basic auth if not doing digest auth. */
	...
	if(authorization_header) // add space for authorization_header if needed
	{
		ix+=strlen(authorization_header);
	}
	...
	if(authorization_header) // add space for authorization_header if needed
	{
		strcat(netcam->connect_request,authorization_header);
	...
	MOTION_LOG(DBG, TYPE_NETCAM, NO_ERRNO,_("generated header (%s)"), netcam->connect_request);
